...
<Image src="/images/AFMI-Logo.png" alt="AFM Shalom logo" fill style={{ objectFit: 'contain' }} />
</div>
<div>
<div className="font-semibold">AFM Shalom Center</div>
<div className="text-xs text-slate-500">Established for growth, expansion and increase</div>
</div>
</Link>
...
{nav.map((n) => (
<Link key={n.href} href={n.href} className="text-slate-700 hover:text-primary transition">
{n.label}
</Link>
))}
</nav>
...
{nav.map((n) => (
<Link key={n.href} href={n.href} onClick={() => setOpen(false)} className="block text-slate-700 py-2">
{n.label}
</Link>
))}
</div>
